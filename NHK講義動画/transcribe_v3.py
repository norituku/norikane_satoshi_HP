#!/usr/bin/env python3
"""gemini-3-flash-preview で残りチャンクを文字起こし"""
...
# v02 chunk00-06 (no transcripts yet)
MIN_SIZE = 2_000  # chars minimum for a valid transcript


def get_chunks_to_process():
    chunks = []
    for f in sorted(CHUNKS_DIR.glob("*v02*chunk*.mp3")):
        out = f.with_name(f.stem + "_transcript.txt")
...
def main():
    ...
    print(f"\n=== {len(chunks)} v02 chunks to process with {MODEL} ===\n", flush=True)
    if not chunks:
    ...
    for i, chunk_path in enumerate(chunks):
        ...
        for attempt in range(5):
            try:
                ...
                if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                    ...
                    wait = 30
                    print(f"  TIMEOUT (attempt {attempt+1}/5), retrying in {wait}s...", flush=True)
                    time.sleep(wait)
                else:
        ...
        if i < len(chunks) - 1:
            print("  Cooling down 30s...", flush=True)
            time.sleep(30)
