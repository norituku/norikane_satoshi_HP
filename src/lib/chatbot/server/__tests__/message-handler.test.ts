...
describe("handleChatbotMessage user context", () => {
  ...
  it.each(["あなたの名前は？", "AIアシスタントの名前は？", "このチャットの名前は？"])(
    ...
    async (prompt) => {
      ...
      expect(result.tier).toBe("tier-1-hosted-chrome-notion-ai")
      expect(result.ui).toEqual({ kind: "none" })
  ...
  it("posts an edit event and regenerated response into the stored Slack thread", async () => {
    ...
    harness.generate.mockResolvedValue({ rawText: customerReply("再生成後の返信です"), tier: "tier-1-hosted-chrome-notion-ai" })
  ...
  it("keeps edited regeneration in the same Slack thread after stale server edit fallback", async () => {
    ...
    harness.generate.mockResolvedValue({ rawText: customerReply("復元後の返信です"), tier: "tier-1-hosted-chrome-notion-ai" })
    harness.slackNotifier.mockResolvedValue({ status: "sent", ts: "1700000000.000200" })
  ...
  it("recovers only labeled booking prefill from user messages", async () => {
    const harness = setup({
      existingConversation: conversation({
        messages: [
          ...
          message("user", "担当者名:テスト太郎、会社名:テスト株式会社、メールは client@example.com です。"),
        ],
        context: {
          sessionId: "session_1",
          userId: "user_a",
  ...
  it("stores labeled prefill without starting booking confirmation", async () => {
    const harness = setup({
      existingConversation: conversation({
        messages: [message("assistant", "ご相談内容を教えてください。")],
        context: {
          sessionId: "session_1",
          userId: "user_a",
    ...
    harness.generate.mockResolvedValueOnce({
      rawText: customerReply("必要な確認を続けます。"),
      tier: "tier-1-hosted-chrome-notion-ai",
    })
    ...
    await handleChatbotMessage(
      {
        sessionId: "session_1",
        userId: "user_a",
        message: "案件名は「案件T」、氏名はテスト太郎、会社名はテスト株式会社、メールアドレスは client@example.com です。",
      },
      harness.options,
    )
  ...
  it("forces contextual project length choices when Tier1 asks the duration as free text", async () => {
    const harness = setup()
    harness.generate.mockResolvedValueOnce({
  ...
  it("ignores non-tier2 proposed routing decisions and keeps talking when there is no tool call", async () => {
    const harness = setup()
    harness.generate.mockResolvedValueOnce({
  ...
  it("posts the first Slack conversation notification as a parent message and saves the returned thread ts", async () => {
    const harness = setup()
    harness.generate.mockResolvedValue({ rawText: customerReply("返信です"), tier: "tier-1-hosted-chrome-notion-ai" })
  ...
  it("posts a problem notification into the same thread for tier3 fallback", async () => {
    const harness = setup({
      existingConversation: conversation({
        context: { sessionId: "session_1", userId: "user_a", slackThreadTs: "1700000000.000100" },
      }),
  ...
  it("does not post a problem notification for successful tier1 responses", async () => {
    const harness = setup({
      existingConversation: conversation({
        context: { sessionId: "session_1", userId: "user_a", slackThreadTs: "1700000000.000100" },
      }),
    ...
    await handleChatbotMessage(
      { sessionId: "session_1", userId: "user_a", message: "Tier1通常応答" },
      harness.options,
    )
  ...
  it("passes only safe retry diagnostics to Slack conversation notifications", async () => {
    ...
    await handleChatbotMessage(
      { sessionId: "session_1", userId: "user_a", message: "Tier1 retry diagnostics" },
      harness.options,
    )
  ...
  it("posts a problem notification when a response falls below hosted tier1", async () => {
    const harness = setup({
      existingConversation: conversation({
        context: { sessionId: "session_1", userId: "user_a", slackThreadTs: "1700000000.000100" },
      }),
    ...
    await handleChatbotMessage(
      { sessionId: "session_1", userId: "user_a", message: "Tier2に落ちるケース" },
      harness.options,
    )
  ...
  it("passes the same safe retry diagnostics to tier3 issue notifications", async () => {
    const harness = setup({
      existingConversation: conversation({
        context: { sessionId: "session_1", userId: "user_a", slackThreadTs: "1700000000.000100" },
      }),
    ...
    await handleChatbotMessage(
      { sessionId: "session_1", userId: "user_a", message: "Tier3 diagnostics" },
      harness.options,
    )
