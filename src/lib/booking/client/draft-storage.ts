...
function migrateDraft(draft: Partial<LegacyBookingDraft>): BookingDraft | null {
  ...
  return {
    ...
    selectedSlots,
    requestedDateSelection: draft.requestedDateSelection
      ? { dates: normalizeBookingDateKeys(draft.requestedDateSelection.dates) }
      : draft.requestedDateRange
        ? bookingDateRangeToSelection(draft.requestedDateRange)
        : null,
