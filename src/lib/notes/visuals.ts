...
export const VISUAL_REGISTRY: Record<string, VisualConfig> = {
  ...
  "correction-reversibility": {
    ...
    caption:
      "ゲイン×ガンマ（左）とリフト×ガンマ（右）を 1 ノードずつ 7 層まで積み上げ（往路）、続けて符号を反転した戻し操作を同順序で 7 層積み足す（復路）。14 層目で y = x 理想復元線を点線で重ね描きし、実線とのズレが「戻りきらなかった残差 = くすみ」として残る。14 秒で 1 ループ。",
    alt: "ゲイン×ガンマとリフト×ガンマの2列比較。各セル7層を往路で積み、復路7層で逆操作を同順序で積み足す14層構成。14層目で y=x 理想線を点線重ねし、実線とのズレで戻りきらない残差を可視化するアニメーション。14秒1ループ。",
