...
export const VISUAL_REGISTRY: Record<string, VisualConfig> = {
  ...
  "correction-space-choice": {
    ...
    caption:
      "Log・Linear・Gamma の 3 セルを横並びに置き、各空間で自然な操作（オフセット / ゲイン / ゲイン）を同じ強さで揺らしたときのトーンカーブの応答を 6.5 秒 1 ループで比較。各セルに「物理（光物理に乗るか）」と「レンジ（0..1 信号レンジに収まるか）」の二判定バッジを置き、Log は物理 △ / レンジ ○、Linear は物理 ○ / レンジ ✗、Gamma は物理 ○ / レンジ ○ となる構図を可視化する。Log セルの「物理 △」横にはアテンションバッジ (i) を置き、クリックで「ACEScは物理 ○、ACEScct のときは ○△」の補足を展開する。",
    alt: "Log・Linear・Gamma の 3 空間でそれぞれオフセット / ゲイン / ガンマを同位相で揺らし、トーンカーブが 0..1 の信号レンジ内外でどう振る舞うかを比較するアニメーション。各セル下部に物理 / レンジの二判定バッジと数値 readout を 1 行に並べる。6.5 秒 1 ループ。",
    aspect: { width: 1500, height: 500 },
