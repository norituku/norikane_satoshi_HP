...
export const VISUAL_REGISTRY: Record<string, VisualConfig> = {
  ...
  "grading-look-decomposition": {
    ...
    title: "言葉 → 4 軸の足場",
    caption:
      "現場の 4 つの言葉が、色の広がり・転がり / 濃度 / カーブ / RGB カラーバランスの対応軸へ落ちる。右側は全軸共通の 12 chip テストチャートで、刺さった軸だけが hue / saturation、輝度、S 字コントラスト、RGB 偏りとして反応する。12 秒で 1 ループ。",
    alt: "監督や DP の言葉が 4 つの知覚軸のどこに落ちるかを示し、右側の各軸に同じ 12 chip テストチャートを並べて、矢印が刺さった軸だけが反応し他の 3 軸は静止するループアニメーション",
    aspect: { width: 16, height: 5 },
  ...
  "grading-secret-pantry": {
    ...
    title: "秘伝のタレを重ねる",
    caption:
      "左右の棚に蓄積された 4 軸の因数別パーツが、中央の新作 Log プレビューへ順番に重なる。仮プレースホルダの 5 枚の色面をクロスフェードし、素のフレームが完成ルックへ育つ過程を 11 秒で 1 ループ。",
